...
class XslxSheetHandler extends DefaultHandler with Logging {
	...
	var memberRelationships: JList[RawMemberRelationship] = _
	...
	def this(sst: SharedStringsTable, memberRelationships: JList[RawMemberRelationship]) = {
		this();
		this.sst = sst;
		...
		name match {
			case "c" => {
				if (!isFirstRow) {
					...
					columnIndex match {
						case 1 => currentRawMemberRelationship.subjectUniversityId = lastContents
						case 2 => currentRawMemberRelationship.agentUniversityId = lastContents
						case 3 => currentRawMemberRelationship.agentName = lastContents
						case _ =>
					}
				}
			}
			case "row" => {
				if (isFirstRow)
					...
					memberRelationships.add(currentRawMemberRelationship)
			}
			case _ =>
		}
	}
