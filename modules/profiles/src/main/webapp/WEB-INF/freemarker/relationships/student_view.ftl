...
<#--
<#assign submit_url><@routes.filter_students  /></#assign>  -->

			<@f.form commandName="viewRelatedStudentsCommand" action="/profiles/tutor/students" method="GET" cssClass="form-inline">
				...
				<div class="btn-group-group">
					...
					<@filter "courseTypes" "Course type" current_course_types_filter viewRelatedStudentsCommand.allCourseTypes; courseType>
						<input type="checkbox" name="${status.expression}" value="${courseType.code}" data-short-value="${courseType.code}" ${contains_by_code(viewRelatedStudentsCommand.courseTypes, courseType)?string('checked','')}>
					${courseType.description}
					</@filter>

					<#assign current_routes_filter><@current_filter_value "routes"; route>${route.code?upper_case}</@current_filter_value></#assign>
					<@filter "routes" "Route" current_routes_filter viewRelatedStudentsCommand.allRoutes; route>
						<input type="checkbox" name="${status.expression}" value="${route.code}" data-short-value="${route.code?upper_case}" ${contains_by_code(viewRelatedStudentsCommand.routes, route)?string('checked','')}>
					...
					<@filter "modesOfAttendance" "Attendance" current_moa_filter viewRelatedStudentsCommand.allModesOfAttendance; moa>
						<input type="checkbox" name="${status.expression}" value="${moa.code}" data-short-value="${moa.shortName?capitalize}" ${contains_by_code(viewRelatedStudentsCommand.modesOfAttendance, moa)?string('checked','')}>
					${moa.fullName}
					</@filter>

					<#assign current_years_filter><@current_filter_value "yearsOfStudy"; year>${year}</@current_filter_value></#assign>
					<@filter "yearsOfStudy" "Year of study" current_years_filter viewRelatedStudentsCommand.allYearsOfStudy; yearOfStudy>
						<input type="checkbox" name="${status.expression}" value="${yearOfStudy}" data-short-value="${yearOfStudy}" ${viewRelatedStudentsCommand.yearsOfStudy?seq_contains(yearOfStudy)?string('checked','')}>
					${yearOfStudy}
					</@filter>
					...
					<@filter "sprStatuses" "Status" current_statuses_filter viewRelatedStudentsCommand.allSprStatuses; sprStatus>
						<input type="checkbox" name="${status.expression}" value="${sprStatus.code}" data-short-value="${sprStatus.shortName?capitalize}" ${contains_by_code(viewRelatedStudentsCommand.sprStatuses, sprStatus)?string('checked','')}>
					${sprStatus.fullName}
					</@filter>

					<#assign current_modules_filter><@current_filter_value "modules"; module>${module.code?upper_case}</@current_filter_value></#assign>
					<@filter "modules" "Module" current_modules_filter viewRelatedStudentsCommand.allModules; module>
						<input type="checkbox" name="${status.expression}" value="${module.code}" data-short-value="${module.code?upper_case}" ${contains_by_code(viewRelatedStudentsCommand.modules, module)?string('checked','')}>
		...
		<div id="filter-results">
			<#include "student_view_results.ftl" />
		</div>
...
<script type="text/javascript">
	jQuery(function($) {
		...
		$('#viewRelatedStudentsCommand input').on('change', function(e) {
		...
		$('.clear-filter').on('click', function() {
			...
			doRequest($('#viewRelatedStudentsCommand'));
		});
	});
</script>
