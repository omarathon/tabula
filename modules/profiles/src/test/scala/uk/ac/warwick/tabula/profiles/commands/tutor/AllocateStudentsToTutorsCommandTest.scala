...
class AllocateStudentsToTutorsCommandTest extends TestBase with Mockito {
	...
	@Test def itWorks = withUser("boombastic") {
		...
		val rel1 = StudentRelationship(staff1.universityId, RelationshipType.PersonalTutor, student1.universityId + "/1")
		val rel2 = StudentRelationship(staff1.universityId, RelationshipType.PersonalTutor, student2.universityId + "/1")
		val rel3 = StudentRelationship(staff2.universityId, RelationshipType.PersonalTutor, student3.universityId + "/1")
		...
		service.listStudentRelationshipsByDepartment(RelationshipType.PersonalTutor, department) returns (Seq(rel1, rel2, rel3))
		service.listStudentsWithoutRelationship(RelationshipType.PersonalTutor, department) returns (Seq(student4, student5, student6, student7))
		
		val cmd = new AllocateStudentsToTutorsCommand(department, currentUser)
		cmd.service = service
		...
		cmd.additionalTutors = JArrayList(staff3.userId)
		cmd.onBind(null)
