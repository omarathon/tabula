...
class ViewStudentPersonalTimetableCommandTest extends TestBase with Mockito{
	...
	val event = TimetableEvent("","","",TimetableEventType.Induction,Nil,DayOfWeek.Monday,LocalTime.now, LocalTime.now,None,None,Nil, AcademicYear(2012))
	...
	scheduledMeetingEventSource.occurrencesFor(testStudent, user, TimetableEvent.Context.Staff) returns meetingOccurrences
