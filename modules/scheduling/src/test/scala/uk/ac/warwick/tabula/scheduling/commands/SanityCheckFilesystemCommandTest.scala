...
class SanityCheckFilesystemCommandTest extends TestBase with Mockito {
	...
	cmd.fileHasher = new SHAFileHasher
	...
	@Test def itWorks = withFakeTime(new DateTime(2013, DateTimeConstants.JANUARY, 13, 15, 18, 2, 278)) {
		...
		dao.getFileById("file1") returns (Some(file))
		dao.getFileById("file2") returns (Some(file))
		dao.getFileById("file3") returns (Some(file))
		dao.getFileById("file4") returns (Some(file))
		dao.getFileById("file5") returns (Some(file))
