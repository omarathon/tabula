...
class ImportStudentRowCommandTest extends TestBase with Mockito with Logging {
	...
	@Test def testCaptureTutorIfSourceIsLocal {
		...
		new Environment {
			...
			there was no(relationshipService).replaceStudentRelationship(tutorRelationshipType, "0672089/2", Seq("0070790"))
		}
	}
	...
	@Test def testCaptureTutorIfSourceIsSits {
		...
		new Environment {
			...
			there was one(relationshipService).replaceStudentRelationship(tutorRelationshipType, "0672089/2", Seq("0070790"))
		}
	}
}
