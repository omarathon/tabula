...
object SmallGroupsReportFilters {
	...
	def unrecorded(result: AllSmallGroupsReportCommandResult): AllSmallGroupsReportCommandResult = {
		if (result.eventWeeks.isEmpty)
			result
		else {
			val thisWeek = termService.getAcademicWeekForAcademicYear(DateTime.now, result.eventWeeks.head.event.group.groupSet.academicYear)
