...
class PersonalTutorRoleProviderTest extends TestBase with Mockito {
	...
	@Test def isTutor = withUser("cuscav", "0123456") {
		...
		relationshipService.listStudentRelationshipsWithUniversityId(RelationshipType.PersonalTutor, "0123456") returns (Seq(rel1, rel2))

		provider.getRolesFor(currentUser, member) should be (Seq(PersonalTutor(member)))
	}

	@Test def notTutor = withUser("cuscav", "0123456") {
		relationshipService.listStudentRelationshipsWithUniversityId(RelationshipType.PersonalTutor, "0123456") returns (Seq())
