...
import uk.ac.warwick.tabula.commands.attendance.old.PermissionsAwareRoutes
...
import uk.ac.warwick.tabula.commands.attendance.manage.old.{AddMonitoringPointSetState, AddMonitoringPointSetValidation, AddMonitoringPointSetCommand}
...
class AddMonitoringPointSetCommandTest extends TestBase with Mockito {
	...
	trait Fixture {
		...
		val thisAcademicYear = AcademicYear.guessSITSAcademicYearByDate(new DateTime().withYear(2013))
