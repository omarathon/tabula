...
class CachedTimetableFetchingServiceTest  extends TestBase with Mockito{
	...
	private trait Fixture {
		...
		val studentEvents = Seq(new TimetableEvent("test","test","test","test",TimetableEventType.Lecture,Nil,DayOfWeek.Monday,new LocalTime,new LocalTime,None,TimetableEvent.Parent(None),None,Nil,Nil, AcademicYear(2013),RelatedUrl("",None)))
	...
	def keyTypesAreDiscriminated() { new Fixture {
		...
		val staffEvents = Seq(new TimetableEvent("test2", "test2", "test2","test2",TimetableEventType.Lecture,Nil,DayOfWeek.Monday,new LocalTime,new LocalTime,None,TimetableEvent.Parent(None),None,Nil,Nil, AcademicYear(2013),RelatedUrl("",None)))
