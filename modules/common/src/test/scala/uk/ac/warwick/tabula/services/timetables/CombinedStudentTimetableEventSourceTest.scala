...
class CombinedStudentTimetableEventSourceTest extends TestBase with Mockito{
	...
	val ttEvent= TimetableEvent("", "From Timetable","", "",TimetableEventType.Induction,Nil,DayOfWeek.Monday,LocalTime.now, LocalTime.now,None,Parent.Empty(),None,Nil,Nil,AcademicYear(2013))
	val timetableEvents = Seq(ttEvent)

	val sgEvent= TimetableEvent("", "From Group","", "",TimetableEventType.Induction,Nil,DayOfWeek.Monday,LocalTime.now, LocalTime.now,None,Parent.Empty(),None,Nil,Nil,AcademicYear(2013))
