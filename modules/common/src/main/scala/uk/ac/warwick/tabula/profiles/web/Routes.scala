...
object Routes {
	...
	object Profile {
		...
		def students(department: Department, academicYear: AcademicYear) = context + "/department/%s/students/%s" format(encoded(department.code), encoded(academicYear.startYear.toString))
