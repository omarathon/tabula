...
object TurnitinLti {
	...
	def validFileType(file: FileAttachment): Boolean =
		TurnitinLti.validExtensions contains getExtension(file.name).toLowerCase
...
/**
 * Service for accessing the TurnitinLti plagiarism API.
 *
 ...
 */
@Service
class TurnitinLti extends Logging with DisposableBean with InitializingBean {


	/** The top level account ID (usually for University of Warwick account) */
	@Value("${TurnitinLti.aid}") var aid: String = null
	/** Sub-account ID underneath University of Warwick */
	@Value("${TurnitinLti.said}") var said: String = null
	...
	@Value("${TurnitinLti.url}") var apiEndpoint: String = _
	...
	def login(user: CurrentUser): Option[TurnitinLtiSession] = {
		login( user.email, user.firstName, user.lastName )
	}
	
	def login(email:String, firstName:String, lastName:String): Option[TurnitinLtiSession] = {
		val session = new TurnitinLtiSession(this, null)
		val userEmail = if (email == null || email.isEmpty) firstName + lastName + "@TurnitinLti.warwick.ac.uk" else email
		...
		session.login() match {
			case Created(sessionId) if sessionId != "" => {
				val session = new TurnitinLtiSession(this, sessionId)
				session.userEmail = userEmail
				session.userFirstName = firstName
				session.userLastName = lastName
