...
case class TurnitinLtiResponse(
	...
	def message = TurnitinLtiResponse.responseCodes.getOrElse(code, unknownMessage)
...
object TurnitinLtiResponse extends Logging {
	...
	def fromDiagnostic(string: String) = {
		...
		new TurnitinLtiResponse(code, diagnostic = Some(string))
	}
	
	def redirect(location: String) = {
		new TurnitinLtiResponse(0, redirectUrl = Some(location))
	}

	def fromXml(xml: Elem) = {
		new TurnitinLtiResponse((xml \\ "rcode").text.toInt, xml = Some(xml))
	}
