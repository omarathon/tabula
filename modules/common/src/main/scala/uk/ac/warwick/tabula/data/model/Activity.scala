 ...
 */
class Activity[A](val title: String, val date: DateTime, val priority: Double, val agent: User, val url: String, val urlTitle: String, val verb: String, val message: String, val entity: A, target: AnyRef=null) {
		...
		event.eventType match {
			case "SubmitAssignment" if (event.hasProperty("submission")) => {
				...
				if (submission.isDefined) {
					...
					val activity = new Activity[Any](title, date, 0, agent, null, null, "submit", "", submission.get)
					Option(activity)
				} else {
					val title = "New submission (since deleted)"
					val date = event.eventDate
					val agent = userLookup.getUserByUserId(event.userId)
					val activity = new Activity[Any](title, date, 0, agent, null, null, "submit", "", Nil)
					Option(activity)
				}
			}
			case _ => {
