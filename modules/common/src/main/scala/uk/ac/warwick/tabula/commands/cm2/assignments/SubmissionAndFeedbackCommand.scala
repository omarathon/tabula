	...
	override def applyInternal(): SubmissionAndFeedbackResults = {
		...
		val unsubmitted = benchmarkTask("Get unsubmitted users") {
			for (user <- unsubmittedMembers) yield {
				...
				val progress = cm2WorkflowService.progress(assignment)(cm2)
		...
		val membersWithPublishedFeedback = submitted.filter { student =>
			student.cm2.enhancedFeedback exists { _.feedback.checkedReleased }
		}
		...
		val stillToDownload = membersWithPublishedFeedback.filterNot(_.cm2.enhancedFeedback.exists(_.downloaded))
