...
case class GotSubmissions(list: Seq[TurnitinLtiSubmissionInfo]) extends SuccessResponse("Got submissions")
 ...
 *
 */
trait TurnitinLtiMethods { self: TurnitinLtiSession =>
	...
	def login() = {
		...
		val response = doRequestAdvanced(None,
			"fcmd" -> "2",
			"utp" -> "2",
			"uem" -> userEmail,
			"ufn" -> userFirstName,
	...
	def createAssignment(classId: ClassId, className: ClassName, assignmentId: AssignmentId, assignmentName: AssignmentName, department: Department): Response = {
		...
		val response = doRequest(None, params: _*)
	...
	private def resolveAssignmentResponse(response: TurnitinLtiResponse) = {
	...
	def submitPaper(
		...
		val response = doRequest(Some(FileData(file, filename)),
			"cid" -> classId.value,
			"ctl" -> className.value,
			"assignid" -> assignmentId.value,
...
//		Failed(response.code, response.message)
	...
	def resolveError(response: TurnitinLtiResponse): Response = response.code match {
