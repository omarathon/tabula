...
import uk.ac.warwick.tabula.data.model.{IdEquality, Notification}
...
@Entity
class RecipientNotificationInfo extends IdEquality {

	def this(notification: Notification[_,_], recipient: User) {
		this()
		this.notification = notification
		this.recipient = recipient
		this.id = s"${notification.id} - ${recipient.getUserId}"
	}
