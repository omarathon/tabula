...
class RelationshipServiceImpl extends RelationshipService with Logging {
	...
	def studentDepartmentMatchesAndExpectedToHaveRelationship(relationshipType: StudentRelationshipType, department: Department)(member: StudentMember) = {
		...
		.filter(scd => Option(scd.route.adminDepartment).exists(adminDepartment => adminDepartment == department || adminDepartment == department.rootDepartment)) // there needs to be an SCD for the right department ...
