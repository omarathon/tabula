 ...
 */
@Controller
@RequestMapping(value=Array("/profiles/department/{department}/{relationshipType}/allocate-old"))
class OldAllocateStudentsToRelationshipController extends ProfilesController {
	...
	def form(cmd: OldAllocateStudentsToRelationshipCommand) = {
		cmd.sort()
		Mav("profiles/relationships/allocate-old")
	}
	...
	def previewFileUpload(@PathVariable department: Department, @Valid cmd: OldAllocateStudentsToRelationshipCommand, errors: Errors): Mav = {
		if (errors.hasErrors && errors.getFieldErrors.asScala.exists { _.getCode == "file.wrongtype.one" }) {
			...
			Mav("profiles/relationships/upload_preview-old")
		}
	}
	...
	def seekConfirmation(@Valid cmd: OldAllocateStudentsToRelationshipCommand, errors: Errors): Mav = {
		if (errors.hasErrors) {
			...
			Mav("profiles/relationships/seek_confirmation-old",
