...
class LegacyTimetableController extends BaseController {
	...
	@RequestMapping
	def redirectCurrentUser(user: CurrentUser) =
		user.profile match {
	...
	def redirectMember(@PathVariable member: Member, request: HttpServletRequest) = {
	...
	def redirectApi(@RequestParam(value="whoFor") whoFor: Member, request: HttpServletRequest) = {
	...
	def redirectIcs(@PathVariable timetableHash: String, request: HttpServletRequest) = {
	...
	def redirectIcal(@RequestParam("timetableHash") timetableHash: String, request: HttpServletRequest) = {
		val r = new RedirectView(toplevelUrl + Routes.api.timetables.calendarICalForHash(mandatory(timetableHash)) + request.getQueryString.maybeText.map("?" + _).getOrElse(""))
		r.setStatusCode(HttpStatus.MOVED_PERMANENTLY)
		r
	}
	...
	def redirectIcalForMember(@PathVariable member: Member, request: HttpServletRequest) = {
