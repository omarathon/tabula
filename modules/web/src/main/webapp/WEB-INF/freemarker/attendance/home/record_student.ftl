										...
										<div class="pull-right">
											...
											<#if features.attendanceMonitoringNote>
												...
												<#if hasNote>
													<#local note = mapGet(command.attendanceNotes, point) />
													<#if note.hasContent>
														<a id="attendanceNote-${student.universityId}-${point.id}" class="btn use-tooltip attendance-note edit" title="Edit attendance note" href="<@routes.attendance.editNote student=command.student point=point returnTo=((info.requestedUri!"")?url) />"><i class="icon-edit attendance-note-icon"></i></a>
													<#else>
														<a id="attendanceNote-${student.universityId}-${point.id}" class="btn use-tooltip attendance-note" title="Add attendance note" href="<@routes.attendance.editNote student=command.student point=point returnTo=((info.requestedUri!"")?url) />"><i class="icon-edit attendance-note-icon"></i></a>
													</#if>
												<#else>
													<a id="attendanceNote-${student.universityId}-${point.id}" class="btn use-tooltip attendance-note" title="Add attendance note" href="<@routes.attendance.editNote student=command.student point=point returnTo=((info.requestedUri!"")?url) />"><i class="icon-edit attendance-note-icon"></i></a>
												</#if>
											...
											<#if point.pointType?? && point.pointType.dbValue == "meeting">
												<a class="meetings" title="Meetings with this student" href="<@routes.attendance.studentMeetings point command.student />"><i class="icon-info-sign icon-fixed-width"></i></a>
											<#else>
