		...
		<form id="recordAttendance" action="" method="post" class="dirty-check">
			...
			<#macro controls pointCheckpointPair>
				...
				<#if mapGet(attendanceNotes, point)??>
					...
					<#if note.hasContent>
						<a id="attendanceNote-${student.universityId}-${point.id}" class="btn use-tooltip attendance-note edit" title="Edit attendance note ba!" href="<@routes.attendance.noteEdit academicYear.startYear?c student point />?dt=${.now?string('iso')}">
