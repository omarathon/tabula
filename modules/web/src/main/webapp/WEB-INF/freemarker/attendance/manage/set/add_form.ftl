...
<form id="addMonitoringPointSet" action="<@routes.attendance.createSet command.dept command.academicYear />" method="POST" class="modify-monitoring-points">
	...
	<div class="row-fluid">
		...
		<div class="span8">
			<a href="<@routes.attendance.addPoint command.dept />?form=true" class="btn btn-primary new-point"><i class="icon-plus"></i> Create new point</a>
		</div>
	</div>

	<#include "_monitoringPoints.ftl" />

	<input type="submit" value="Create" class="btn btn-primary"/> <a class="btn" href="<@routes.attendance.manageDepartment command.dept />">Cancel</a>
