...
<#if hasSets>
	...
	<h3><a href="<@routes.attendance.viewDepartmentPoints department />">View by point</a></h3>
	<#if can.do("MonitoringPoints.View", department)>
		<#list department.displayedStudentRelationshipTypes as relationshipType>
		<h3><a href="<@routes.attendance.viewDepartmentAgents department relationshipType/>">View by ${relationshipType.agentRole}</a></h3>
		</#list>
	</#if>
<#else>
