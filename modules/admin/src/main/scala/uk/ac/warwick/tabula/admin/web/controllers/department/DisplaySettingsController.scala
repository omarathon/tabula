...
import uk.ac.warwick.tabula.admin.commands.department.{DisplaySettingsCommandInternal, DisplaySettingsCommand, DisplaySettingsCommandState}
...
class DisplaySettingsController extends AdminController {
	...
	validatesSelf[DisplaySettingsCommandInternal]
	...
	def saveSettings(@Valid @ModelAttribute("displaySettingsCommand") cmd:DisplaySettingsCommandInternal with Appliable[Unit], errors:Errors) = {
		if (errors.hasErrors){
