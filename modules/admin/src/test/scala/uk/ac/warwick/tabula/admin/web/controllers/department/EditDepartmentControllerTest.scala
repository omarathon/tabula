...
import uk.ac.warwick.tabula.admin.commands.department.EditDepartmentCommandState
...
class EditDepartmentControllerTest extends TestBase with Mockito {
	...
	@Test def submit {
		...
		controller.submit(command, errors).viewName should be (s"redirect:${Routes.department(department)}")
