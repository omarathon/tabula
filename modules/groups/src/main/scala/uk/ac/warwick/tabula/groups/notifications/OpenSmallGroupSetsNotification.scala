...
class OpenSmallGroupSetsNotification(val agent: User, val recipient: User, val _object: Seq[SmallGroupSet]) extends Notification[Seq[SmallGroupSet]] with SingleRecipientNotification {
	this: TextRenderer =>
