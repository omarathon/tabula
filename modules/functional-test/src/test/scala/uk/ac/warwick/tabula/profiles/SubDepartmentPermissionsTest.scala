...
class SubDepartmentPermissionsTest  extends BrowserTest with GivenWhenThen with FixturesDriver{
	...
	before{
		Given("A department xxx and a sub-department xxx-ug exist")
		...
		And("student1 and student2 have a membership record with an undergraduate course")
			...
			createStudentMember(P.Student1.usercode,routeCode=TEST_UG_ROUTE_CODE, courseCode=TEST_UNDERGRAD_COURSE_CODE,deptCode = TEST_DEPARTMENT_CODE)
 		  createStudentMember(P.Student2.usercode,routeCode=TEST_UG_ROUTE_CODE, courseCode=TEST_UNDERGRAD_COURSE_CODE,deptCode = TEST_DEPARTMENT_CODE)
