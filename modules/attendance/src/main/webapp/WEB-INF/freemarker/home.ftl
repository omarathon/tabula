...
<div class="btn-toolbar dept-toolbar">
	<div class="btn-group dept-settings">
		...
		<ul class="dropdown-menu pull-right">
			<li><a href="<@routes.home '2013' />"><#if academicYear.startYear == 2013><strong>13/14</strong><#else>13/14</#if></a></li>
			<#if features.attendanceMonitoringAcademicYear2014>
				<li><a href="<@routes.home '2014' />"><#if academicYear.startYear == 2014><strong>14/15</strong><#else>14/15</#if></a></li>
			</#if>
			<#if features.attendanceMonitoringAcademicYear2015>
				<li><a href="<@routes.home '2015' />"><#if academicYear.startYear == 2015><strong>15/16</strong><#else>15/16</#if></a></li>
			</#if>
		</ul>
	</div>
</div>
	...
	<ul>
		<#list relationshipTypesMap?keys as relationshipType>
			<#if relationshipTypesMapById[relationshipType.id]>
				...
				<#else>
					<li><h3><a id="relationship-${relationshipType.urlPart}-${academicYear.startYear?c}" href="<@routes.agentHomeForYear relationshipType academicYear.startYear?c />">${relationshipType.studentRole?cap_first}s ${academicYear.label}</a></h3></li>
				</#if>
			</#if>
		</#list>
	</ul>
		...
		<ul class="unstyled">
			<#list viewPermissions as department>
				<li>
					...
					<#else>
						<h3><a id="view-department-${department.code}-${academicYear.startYear?c}" href="<@routes.viewHomeForYear department academicYear.startYear?c />">${department.name} ${academicYear.label}</a></h3>
					</#if>
				</li>
			</#list>
		</ul>
		...
		<ul class="unstyled">
			<#list managePermissions as department>
				<li>
					...
					<#else>
						<h3><a id="manage-department-${department.code}-${academicYear.startYear?c}" href="<@routes.manageHomeForYear department academicYear.startYear?c/>">${department.name} ${academicYear.label}</a></h3>
					</#if>
				</li>
			</#list>
		</ul>
