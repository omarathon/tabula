...
<#macro submission_details submission=""><#compress>
	<#if submission?has_content>: <#compress>
		...
		<#if attachments?size gt 0>
			...
			<a class="long-running" href="<@url page='/coursework/admin/module/${module.code}/assignments/${assignment.id}/submissions/download/${submission.id}/${filename?url}'/>"><#compress>
				${attachments?size}
...
<#macro submission_status submission="" enhancedExtension="" enhancedFeedback="" student="">
	<#if submission?has_content>
		...
		<#if features.disabilityRenderingInProfiles && student.disability??>
