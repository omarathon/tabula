...
class AddAssignmentsCommand(val department: Department, user: CurrentUser) extends Command[Unit] with SelfValidating {
	...
	PermissionCheck(Permission.Assignment.ImportFromExternalSystem(), department)
	...
	def checkPermissions() = {
		...
		if (hasInvalidAssignments) {
			...
			throw new PermissionDeniedException(user, Permission.Assignment.ImportFromExternalSystem(), department)
		}
	}
