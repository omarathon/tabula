...
import uk.ac.warwick.tabula.services.turnitinlti.TurnitinLti._
import uk.ac.warwick.tabula.services.turnitinlti._

object SubmitToTurnitinLtiJob {
	val identifier = "turnitin-lti-submit"
	def apply(assignment: Assignment) = JobPrototype(identifier, Map(
		"assignment" -> assignment.id))
}

/**
 * This job uses the Turnitin *****NEW LTI***** service to upload an assignment's submission attachments,
...
class SubmitToTurnitinLtiJob extends Job
	with TurnitinLtiTrait with NotifyingJob[Seq[TurnitinLtiSubmissionInfo]] with Logging with FreemarkerRendering {
...
//		}
		 ...
		 */
		@tailrec
		private def runCheck(retries: Int): Option[Seq[TurnitinLtiSubmissionInfo]] = {
