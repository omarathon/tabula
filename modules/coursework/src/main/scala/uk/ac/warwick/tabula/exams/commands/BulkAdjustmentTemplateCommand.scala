...
class BulkAdjustmentTemplateCommandInternal(val assessment: Assessment) extends CommandInternal[ExcelView] {
	...
	private def generateWorkbook: XSSFWorkbook = {
		...
		header.createCell(3).setCellValue("Old adjusted mark")
		header.createCell(4).setCellValue("Old adjusted grade")
		...
		assessment.fullFeedback.sortBy(f => memberOrder(f.universityId)).foreach(f => {
			...
			row.createCell(3).setCellValue(f.latestPublicOrPrivateAdjustment.map(_.mark.toString).getOrElse(""))
			row.createCell(4).setCellValue(f.latestPublicOrPrivateAdjustment.flatMap(_.grade.map(_.toString)).getOrElse(""))
		})
