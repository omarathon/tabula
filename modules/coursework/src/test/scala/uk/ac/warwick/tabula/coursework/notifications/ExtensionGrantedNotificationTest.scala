...
import uk.ac.warwick.tabula.coursework.commands.assignments.extensions.notifications.{ExtensionGrantedNotification, ExtensionChangedNotification}
import uk.ac.warwick.tabula.coursework.{ExtensionFixture, MockRenderer}
import org.mockito.Mockito._
import org.mockito.{ArgumentCaptor, Matchers}

class ExtensionGrantedNotificationTest extends TestBase with Mockito {

	val TEST_CONTENT = "test"

	def createNotification(extension: Extension, student: User, actor: User) = {
		val n = new ExtensionGrantedNotification(extension, student, actor) with MockRenderer
	...
	def shouldCallTextRendererWithCorrectTemplate():Unit = new ExtensionFixture {
		...
		verify(n.mockRenderer, times(1)).renderTemplate(
			Matchers.eq("/WEB-INF/freemarker/emails/new_manual_extension.ftl"),
			any[Map[String,Any]])
	}
