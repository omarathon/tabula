...
import uk.ac.warwick.tabula.timetables.RelatedUrl
...
class GetMemberCalendarJsonApiTest extends TestBase with Mockito {
	...
	def colouriseBasedOnModule(){
		...
		val events = for {
			...
			j = i%3}
			yield {
				FullCalendarEvent(s"Test event $i", "", allDay = false, 0L,0L,"","","","","","","","","","","","Module",s"module$j",s"Module $j","", relatedUrl = RelatedUrl("", None))
			}

		// Add a Busy event at the end
		val coloured = FullCalendarEvent.colourEvents(events :+ FullCalendarEvent("Busy", "", allDay = false, 0L,0L,"","","","","","","","","","","","Empty", "", "", "", relatedUrl = RelatedUrl("", None)))
