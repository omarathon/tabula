...
class ModuleDaoTest extends PersistenceTestBase {
  ...
  @Test
  def testStampMissingFromImport(): Unit = transactional { _ =>
    // not stale before, stale now
    val module1 = Fixtures.module("one", "my name is one")
    // not stale before, not stale now
    val module2 = Fixtures.module("two", "my name is two")
    val module3 = Fixtures.module("three", "my name is three")

    val previously = now.minusHours(3)
    // stale before, not stale now
    module3.missingFromImportSince = previously
    val module4 = Fixtures.module("four", "my name is four")

    // stale before, stale now
    module4.missingFromImportSince = previously
    ...
    dao.getByCode("four").get.missingFromImportSince.withZone(DateTimeZone.getDefault) should be(previously)
  }
