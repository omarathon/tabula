...
class ProcessUrkundQueueCommandTest extends TestBase with Mockito {
  ...
  @Test
  def submitReportSuccess(): Unit = withFakeTime(now) {
    new Fixture {
  ...
  @Test
  def submitReportClientError(): Unit = withFakeTime(now) {
    new Fixture {
      report.nextSubmitAttempt = DateTime.now.minusMinutes(1)
      command.urkundService.findReportToSubmit returns Some(report)
  ...
  @Test
  def submitReportServerError(): Unit = withFakeTime(now) {
    new Fixture {
      report.nextSubmitAttempt = DateTime.now.minusMinutes(1)
      command.urkundService.findReportToSubmit returns Some(report)
  ...
  @Test
  def retrieveReportSuccessAnalysed(): Unit = withFakeTime(now) {
    new Fixture {
  ...
  @Test
  def retrieveReportSuccessRejected(): Unit = withFakeTime(now) {
    new Fixture {
      report.nextResponseAttempt = DateTime.now.minusMinutes(1)
      command.urkundService.findReportToSubmit returns None
      command.urkundService.findReportToRetreive returns Some(report)
  ...
  @Test
  def retrieveReportSuccessError(): Unit = withFakeTime(now) {
    new Fixture {
      report.nextResponseAttempt = DateTime.now.minusMinutes(1)
      command.urkundService.findReportToSubmit returns None
      command.urkundService.findReportToRetreive returns Some(report)
  ...
  @Test
  def retrieveReportSuccessAccepted(): Unit = withFakeTime(now) {
    new Fixture {
      report.nextResponseAttempt = DateTime.now.minusMinutes(1)
      command.urkundService.findReportToSubmit returns None
      command.urkundService.findReportToRetreive returns Some(report)
  ...
  @Test
  def retrieveReportSuccessSubmitted(): Unit = withFakeTime(now) {
    new Fixture {
      report.nextResponseAttempt = DateTime.now.minusMinutes(1)
      command.urkundService.findReportToSubmit returns None
      command.urkundService.findReportToRetreive returns Some(report)
  ...
  @Test
  def retrieveReportClientError(): Unit = withFakeTime(now) {
    new Fixture {
      report.nextResponseAttempt = DateTime.now.minusMinutes(1)
      command.urkundService.findReportToSubmit returns None
      command.urkundService.findReportToRetreive returns Some(report)
  ...
  @Test
  def retrieveReportServerError(): Unit = withFakeTime(now) {
    new Fixture {
      report.nextResponseAttempt = DateTime.now.minusMinutes(1)
      command.urkundService.findReportToSubmit returns None
      command.urkundService.findReportToRetreive returns Some(report)
  ...
  @Test
  def retrieveReportSuccessAnalysedNotCompleteAssignment(): Unit = withFakeTime(now) {
    new Fixture {
      report.nextResponseAttempt = DateTime.now.minusMinutes(1)
      command.urkundService.findReportToSubmit returns None
      command.urkundService.findReportToRetreive returns Some(report)
