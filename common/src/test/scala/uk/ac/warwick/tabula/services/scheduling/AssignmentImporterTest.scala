  ...
  @Test def allAssessmentComponents(): Unit = {
    withFakeTime(dateTime(2012, 5)) {
      ...
      components.map(_.toString()) should be(Seq(
        ...
        "AssessmentComponent[moduleCode=XX101-30,assessmentGroup=A,sequence=A01,inUse=true,module=null,name=Danger Zone,assessmentType=SummerExam,marksCode=null,rawWeighting=50,examPaperCode=Some(XX1010),examPaperTitle=Some(Danger Zone),examPaperSection=Some(n/a),examPaperDuration=Some(PT5400S),examPaperReadingTime=Some(PT900S),examPaperType=Some(OpenBook),finalChronologicalAssessment=true]"
      ))
    }
  }
