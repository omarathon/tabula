...
import uk.ac.warwick.tabula.data.model.{AssessmentType, ModuleRegistration, ModuleSelectionStatus, StudentCourseDetails, UpstreamAssessmentGroup}
...
class ModuleRegistrationServiceTest extends TestBase with Mockito {
  ...
  trait GraduationBenchmarkFixture extends Fixture {
    ...
    val academicYear = AcademicYear(2019)
    ...
    val components = SortedMap(
      "in301" -> Seq(Fixtures.assessmentComponent(modules("in301"), 1, AssessmentType.Essay, 100)),
      ...
      "in306" -> Seq( Fixtures.assessmentComponent(modules("in306"), 1, AssessmentType.SummerExam, 100) )
    )
  ...
  def benchmarkComponentsAndMarks: Unit = {
    new GraduationBenchmarkFixture {
      val componentAndMarks = service.benchmarkComponentsAndMarks(moduleRegistrations.head)
