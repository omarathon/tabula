...
class EditMeetingRecordCommandTest extends TestBase with Mockito {
  ...
  @Test
  def creatorEditMeeting() = withUser(thisCreator.userId) {
  ...
  @Test
  def editMeetingAddedParticipantNeedsToApprove() = withUser(student.userId) {
    withFakeTime(aprilFool) {
  ...
  @Test
  def meetingRecordWorkflow() = withUser("cuslaj") {
    withFakeTime(aprilFool) {
