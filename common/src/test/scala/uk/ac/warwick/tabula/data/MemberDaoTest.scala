...
class MemberDaoTest extends PersistenceTestBase with Logging with Mockito {
	...
	def testGetMemberMissingBefore(): Unit = transactional { tx =>
		...
		val studentsMissingBeforeTwoMonths = memberDao.getMissingBefore[StudentMember](from = new DateTime().minusMonths(2))
		...
		val applicantsMissingBeforeTwoMonths = memberDao.getMissingBefore[ApplicantMember](from = new DateTime().minusMonths(2))
		applicantsMissingBeforeTwoMonths.size should be (2)
		applicantsMissingBeforeTwoMonths should contain allOf("id3", "id4")

		val membersMissingBeforeTwoMonths = memberDao.getMissingBefore[Member](from = new DateTime().minusMonths(2))
