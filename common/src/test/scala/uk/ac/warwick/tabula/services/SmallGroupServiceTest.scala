...
class SmallGroupServiceTest extends TestBase with Mockito {
  ...
  trait Environment {
    ...
    val service = new AbstractSmallGroupService
      ...
      membershipService.determineMembershipUsers(any[Seq[UpstreamAssessmentGroupInfo]], any[Option[UnspecifiedTypeUserGroup]], resitOnly = false) returns Seq(user) ++ Seq(user1) ++ Seq(user2) ++ Seq(user3)
