...
import uk.ac.warwick.tabula.data.model.{AbstractMemberNote, StudentMember}
...
class CreateMemberNoteCommandTest extends TestBase with Mockito {
  ...
  @Test
  def testApply(): Unit = withUser("cuscao", "09876") {
    new ApplyFixture {
  ...
  @Test
  def validMemberNote(): Unit = withUser("cuscao", "09876") {
  ...
  @Test
  def invalidMemberNote(): Unit = withUser("cuscao") {
    new ValidationFixture {
      val errors = new BindException(validator, "command")
