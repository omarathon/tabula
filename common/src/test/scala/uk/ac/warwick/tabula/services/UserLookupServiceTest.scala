...
class UserLookupServiceTest extends TestBase with Mockito {
  ...
  @Test def caching() {
    ...
    unverifiedUser.isVerified should be {
      false
    }
    unverifiedUser.isFoundUser should be {
      false
    }
    ...
    existingUser.isVerified should be {
      true
    }
    existingUser.isFoundUser should be {
      true
    }
    cache.getStatistics.getCacheSize should be(1)
    cache.contains(warwickId) should be {
      true
    }
    ...
    applicantUser.isVerified should be {
      true
    }
    applicantUser.isFoundUser should be {
      false
    }
    cache.getStatistics.getCacheSize should be(1)
    cache.contains(warwickId) should be {
    ...
    anonUser.isVerified should be {
      true
    }
    anonUser.isFoundUser should be {
      false
    }
    cache.getStatistics.getCacheSize should be(1)
    cache.contains(warwickId) should be {
  ...
  @Test def cachingMultiLookups() {
    ...
    cache.getStatistics.getCacheSize should be(0)
    warwickIds.foreach { warwickId => cache.contains(warwickId) should be {
      false
    }
    }
