...
class EditDepartmentCommandTest extends TestBase with FunctionalContextTesting with Mockito {
  ...
  trait Fixture {
    ...
    val department: Department = Fixtures.department("in-ug", "IT Services Undergraduate", isRootDepartment = false)
    department.id = "in-ug"
  ...
  @Test def validateNameDoesntStartWithParent() {
    new Fixture {
      command.code = "in-ug"
  ...
  @Test def validateShortNameTooLong() {
    new Fixture {
      command.code = "in-ug"
