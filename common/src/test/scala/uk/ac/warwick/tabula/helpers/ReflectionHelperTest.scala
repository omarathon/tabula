...
class ReflectionHelperTest extends TestBase with ReflectionsSetup {
  ...
  @Test def allPermissionsTargets(): Unit = {
    ReflectionHelper.allPermissionTargets.contains(classOf[Department]) should be {
      true
    }
  }
  ...
  @Test def notifications(): Unit = {
    ...
    map.contains("SubmissionDueGeneral") should be {
      true
    }
  }

  @Test def allPermissions(): Unit = {
    ReflectionHelper.allPermissions.contains(Permissions.Module.ManageAssignments) should be {
      true
    }
  }

  @Test def groupedPermissions(): Unit = {
    ReflectionHelper.groupedPermissions("Module").contains(("Module.Create", "Module.Create")) should be {
      true
    }
  }
