...
import uk.ac.warwick.tabula.commands.cm2.feedback.{AddFeedbackCommand, FeedbackItem}
...
class AddFeedbackCommandTest extends TestBase with Mockito {
	...
	@Test def duplicateFileNames() = withUser("cuscav") {
		val cmd = new AddFeedbackCommand(assignment, currentUser.apparentUser, currentUser)
