...
class MarkerBulkAdjustmentCommandTest extends TestBase  with Mockito {
	...
	trait TestFixture {
		...
		val students: Seq[User] = 1 to 9 map(i => Fixtures.user(s"$i", s"$i"))
	...
	def commandState() { new TestFixture {
		withUser("1170836", "cuslaj") {
			...
			validForAdjustment(marker1).map(_.student.getWarwickId) should be (Seq("6", "7", "8", "9"))
		}
	}}
	...
	def pointDown() { new TestFixture {
		...
		verify(cmd.feedbackService, times(4)).save(any[MarkerFeedback])
