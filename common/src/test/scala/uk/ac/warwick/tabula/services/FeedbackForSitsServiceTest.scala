...
class FeedbackForSitsServiceTest extends TestBase with Mockito {
  ...
  trait Fixture {
    ...
    gradeGenerator.applyForMarks(Map(feedback._universityId -> feedback.actualMark.get)) returns Map(feedback._universityId -> Seq(GradeBoundary(null, "A", Some(0), Some(100), "N")))
  }
