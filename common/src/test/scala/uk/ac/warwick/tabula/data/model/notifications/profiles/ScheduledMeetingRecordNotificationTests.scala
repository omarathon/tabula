...
class CreateScheduledMeetingRecordNotificationTest extends TestBase with Mockito {
  ...
  def testStudent() {
    new ScheduledMeetingRecordNotificationFixture {
      ...
      notifications.length should be {
        1
      }
      notifications.head.recipients.head should be {
        staff.asSsoUser
      }
    }
  }
...
class EditScheduledMeetingRecordNotificationTest extends TestBase with Mockito
  ...
  def testAdminCreatedByAgent() {
    ...
    notifications2.length should be {
      2
    }
    notifications2(0).recipients.head should be {
      student.asSsoUser
    }
    ...
    notifications2(1).recipients.head should be {
      staff.asSsoUser
    }
