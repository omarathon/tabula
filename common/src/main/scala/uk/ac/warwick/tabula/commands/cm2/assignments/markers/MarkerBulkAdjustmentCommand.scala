...
class MarkerBulkAdjustmentCommandInternal(
	...
	def applyInternal(): Seq[MarkerFeedback] = {
		...
		val toAdjust = validForAdjustment(previousMarker).map(markerFeedback => {
			...
			val amountToAdjust = (adjustmentType match {
				case Percentage => for(mark <- markerFeedback.mark; percent <- percentageAdjustment) yield (mark * (percent / 100)).ceil.toInt
				case Points => pointAdjustment
			}).getOrElse(0)

			markerFeedback.mark  = markerFeedback.mark.map(direction.op(_, amountToAdjust))
			markerFeedback
		})
...
trait MarkerBulkAdjustmentState {
	...
	private val markersFeedback = cm2MarkingWorkflowService.getAllFeedbackForMarker(assignment, marker).getOrElse(currentStage, Seq())
	private val markersStudents = markersFeedback.map(_.student).toSet
	...
	private val previousMarkerFeedback: Map[User, Seq[MarkerFeedback]] = previousMarkers.map(marker => { marker ->
	...
	val skipReasons: Map[User, Map[User, Seq[String]]] = previousMarkers.map(marker => {
	...
	val validForAdjustment: Map[User, Seq[MarkerFeedback]] = previousMarkerFeedback.map { case (marker, markerFeedback) =>
...
case object Up extends MarkerAdjustmentDirection { val value = "Up" ; def op: (Int, Int) => Int = (a:Int, b:Int) => a+b }
case object Down extends MarkerAdjustmentDirection { val value = "Down" ; def op: (Int, Int) => Int = (a:Int, b:Int) => a-b }
