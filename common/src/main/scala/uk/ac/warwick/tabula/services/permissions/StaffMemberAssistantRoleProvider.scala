...
class StaffMemberAssistantRoleProvider extends RoleProvider with TaskBenchmarking {
  ...
  val roleService: MutablePromise[RoleService] = promise {
    Wire[RoleService]
  }
  val roleProviders: MutablePromise[Seq[RoleProvider]] = promise {
    Wire.all[RoleProvider]
  }
  val profileService: MutablePromise[ProfileService] = promise {
    Wire[ProfileService]
  }
