...
class AbstractExportAttendanceToSitsService extends ExportAttendanceToSitsService {
  ...
  def exportToSits(report: MonitoringPointReport): Boolean = {
    ...
    val parameterMap = JHashMap(
      ...
      ("deptCode", Option(report.studentCourseYearDetails).flatMap { scyd => Option(scyd.enrolmentDepartment) }.fold("") {
        _.code
      }.toUpperCase),
      ("courseCode", Option(report.studentCourseDetails).flatMap { scd => Option(scd.course) }.fold("") {
        _.code
      }),
