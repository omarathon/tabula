...
case class ExamGridColumnValueWithTooltip(value: String, actual: Boolean, message: String = "") extends ExamGridColumnValueString(value, actual) {
  override def toHTML: String =
    if (message.hasText)
      s"""<span class="tabula-tooltip" tabindex="0" role="button" data-title="$message">$value</span>"""
    else
      s"""<span>$value</span>"""
}
...
case class ExamGridColumnValueMissing(message: String = "") extends ExamGridColumnValueString("X", isActual = true) {
  override def toHTML: String =
    if (message.hasText)
      s"""<span class="exam-grid-actual-mark tabula-tooltip" tabindex="0" role="button" data-title="$message">X</span>"""
    else
