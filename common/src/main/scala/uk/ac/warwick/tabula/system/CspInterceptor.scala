...
class CspInterceptor extends HandlerInterceptorAdapter with AutowiringTopLevelUrlComponent {
  ...
  def reportingRules(nonce: String): ListMap[String, Option[String]] =
    ListMap(
      ...
      // AJAX request for My Warwick alert unread count and campus-cms for map embeds, and status.warwick.ac.uk for StatusPage summary + GA
      "connect-src" -> Some("'self' https://my.warwick.ac.uk https://campus-cms.warwick.ac.uk https://status.warwick.ac.uk https://ssl.google-analytics.com"),
