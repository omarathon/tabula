...
sealed abstract class CourseworkFilter extends CaseObjectEqualityFixes[CourseworkFilter] {
  ...
  def predicate(parameters: Map[String, String])(student: AssignmentSubmissionStudentInfo): Boolean
...
abstract class ParameterlessCourseworkFilter extends CourseworkFilter {
  def predicate(student: AssignmentSubmissionStudentInfo): Boolean

  final def predicate(parameters: Map[String, String])(student: AssignmentSubmissionStudentInfo): Boolean = predicate(student)

  final def validate(parameters: Map[String, String], fieldName: String)(errors: Errors): Unit = {}
...
object CourseworkFilters {
  ...
  lazy val AllFilters = Seq(
    ...
    SubmissionNotDownloaded, Unsubmitted, NotReleasedForMarking,
  ...
  case object SubmissionNotDownloaded extends ParameterlessCourseworkFilter {
    ...
    def predicate(item: AssignmentSubmissionStudentInfo): Boolean = item.coursework.enhancedSubmission.exists(!_.downloaded)
  ...
  case object SubmittedBetweenDates extends CourseworkFilter {
    ...
    def predicate(parameters: Map[String, String])(item: AssignmentSubmissionStudentInfo): Boolean = {
  ...
  case object NotReleasedForMarking extends ParameterlessCourseworkFilter {
    ...
    def predicate(student: AssignmentSubmissionStudentInfo): Boolean = !student.assignment.isReleasedForMarking(student.user.getUserId)

    def applies(assignment: Assignment): Boolean = assignment.collectSubmissions && assignment.cm2MarkingWorkflow != null
  }
  ...
  case object NoFeedback extends ParameterlessCourseworkFilter {
    ...
    def predicate(item: AssignmentSubmissionStudentInfo): Boolean = item.coursework.enhancedFeedback.forall(_.feedback.isPlaceholder)
