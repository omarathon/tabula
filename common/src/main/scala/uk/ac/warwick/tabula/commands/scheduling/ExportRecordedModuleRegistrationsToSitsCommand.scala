...
abstract class ExportRecordedModuleRegistrationsToSitsCommandInternal
  ...
  override def applyInternal(): Result = transactional() {
    val moduleMarksToUpload = benchmarkTask("Get the 1000 most recently updated module registrations needing writing to SITS") {
    ...
    moduleMarksToUpload.flatMap { student => benchmarkTask(s"Process $student") {
      ...
      val finalAssessmentAttended = benchmarkTask("Determine whether the final assessment has been attended") {
