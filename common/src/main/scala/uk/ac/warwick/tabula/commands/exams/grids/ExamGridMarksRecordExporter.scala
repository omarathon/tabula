			...
			entity.validYears.keys.toSeq.sorted.foreach { yearOfStudy =>
				...
				val yearMark = {
					if (Option(year.studentCourseYearDetails.get.agreedMark).isDefined) {
						...
						progressionService.getYearMark(year.studentCourseYearDetails.get, normalLoadLookup(year.route), routeRulesLookup(year.route, entity.courseDetails.level)).right.toOption
					} else {
						None
					}
				}
				...
				progressionService.suggestedFinalYearGrade(year.studentCourseYearDetails.get, normalLoadLookup(year.route), routeRulesLookup(year.route, entity.courseDetails.level)) match {
