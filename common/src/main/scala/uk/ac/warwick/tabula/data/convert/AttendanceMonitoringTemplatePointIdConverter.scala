...
class AttendanceMonitoringTemplatePointIdConverter extends TwoWayConverter[String, AttendanceMonitoringTemplatePoint] {
  ...
  override def convertRight(id: String): AttendanceMonitoringTemplatePoint = (Option(id) flatMap {
    service.getTemplatePointById
  }).orNull

  override def convertLeft(scheme: AttendanceMonitoringTemplatePoint): String = (Option(scheme) map {
    _.id
  }).orNull
