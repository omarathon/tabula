...
trait NotifiesAffectedGroupMembers extends Notifies[SmallGroupSet, SmallGroupSet] {
  ...
  def createTutorNotification(tutor: User): Option[Notification[SmallGroup, SmallGroupSet]] = {
    ...
    createNotification(set, filteredGroupSet.groups.asScala.toSeq, tutor, new SmallGroupSetChangedTutorNotification, set.emailTutorsOnChange)
  }
  ...
  def createStudentNotification(student: User): Option[Notification[SmallGroup, SmallGroupSet]] = {
    ...
    createNotification(set, filteredGroups.asScala.toSeq, student, new SmallGroupSetChangedStudentNotification, set.emailStudentsOnChange)
  }
  ...
  def emit(set: SmallGroupSet): Seq[Notification[SmallGroup, SmallGroupSet]] = {
    val tutorNotifications = if (set.releasedToTutors) {
      val allEvents = (setBeforeUpdates.groups.asScala.toSeq ++ set.groups.asScala.toSeq).flatMap(g => g.events)
    ...
    val studentNotifications = if (set.releasedToStudents) {
      val allStudents = (setBeforeUpdates.groups.asScala.toSeq ++ set.groups.asScala.toSeq).flatMap(g => g.students.users).distinct
