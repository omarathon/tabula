...
class ImportAddressCommandInternal(member: Member) extends CommandInternal[Unit] with PropertyCopying {
  ...
  def applyInternal(): Unit = {
    ...
    newAddressInfo.currentAddress match {
      case Some(rInfo) if !rInfo.isEmpty => updateAddress("currentAddress", rInfo, member.currentAddress, a => {
        member.currentAddress = a
      })
      case _ => deleteAddress("currentAddress", member.currentAddress, () => {
        member.currentAddress = null
      })
    }

    Option(member).collect { case student: StudentMember =>
      newAddressInfo.hallOfResidence match {
        case Some(rInfo) if !rInfo.isEmpty => updateAddress("termtimeAddress", rInfo, student.termtimeAddress, a => {
          student.termtimeAddress = a
        })
        case _ => deleteAddress("termtimeAddress", student.termtimeAddress, () => {
