...
object SmallGroupSetSelfSignUpState {
  ...
  private[groups] def find(i: String) = members find {
    _.name == i
  }

  def apply(i: String): SmallGroupSetSelfSignUpState = find(i) getOrElse {
    throw new IllegalArgumentException(s"Invalid value for self sign-up state: $i")
  }
}
