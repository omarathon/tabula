...
trait MarkerBulkModerationState {
  ...
  private lazy val previousMarkerFeedback: Map[User, Seq[MarkerFeedback]] = previousMarkers.map(marker => {
    marker ->
...
case object Percentage extends MarkerAdjustmentType {
  val value = "percentage";
  val description = "%"
}

case object Points extends MarkerAdjustmentType {
  val value = "points";
  val description = "Points"
}
...
case object Up extends MarkerAdjustmentDirection {
  val value = "Up";

  def op: (Double, Double) => Double = (a: Double, b: Double) => a + b
}

case object Down extends MarkerAdjustmentDirection {
  val value = "Down";
