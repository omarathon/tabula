...
object ListMarkerFeedbackCommand  {
	def apply(assignment: Assignment, marker: User, submitter: CurrentUser) =
		new ListMarkerFeedbackCommand(assignment, marker, submitter)
...
class ListMarkerFeedbackCommand(val assignment: Assignment, val marker: User, val submitter: CurrentUser)
