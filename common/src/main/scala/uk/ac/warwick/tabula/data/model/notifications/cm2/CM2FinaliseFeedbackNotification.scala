...
object CM2FinaliseFeedbackNotification {
	val templateLocation = "/WEB-INF/freemarker/emails/finalise_feedback_notification.ftl"
}

@Entity
@DiscriminatorValue("CM2FinaliseFeedback")
class CM2FinaliseFeedbackNotification
	...
	override def content = FreemarkerModel(CM2FinaliseFeedbackNotification.templateLocation,
		Map(
			"assignment" -> assignment,
			"finalisedFeedbacks" -> entities
		))

	override def url: String = Routes.cm2.admin.assignment.submissionsandfeedback(assignment)
...
class CM2FinaliseFeedbackNotificationSettings(departmentSettings: NotificationSettings) {
