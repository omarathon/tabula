...
object ImportAssignmentsIndividualYearJob {
      ...
      academicYear.foreach { year =>
        exceptionResolver.reportExceptions {
          EarlyRequestInfo.wrap() {
            ImportAssignmentsCommand.applyIndividualYear(year).apply()
          }
        }
      }
}
...
class ImportAssignmentsIndividualYearJob extends AutowiredJobBean with Logging {

  override def executeInternal(context: JobExecutionContext): Unit =
    ImportAssignmentsIndividualYearJob.execute(features, exceptionResolver) {
