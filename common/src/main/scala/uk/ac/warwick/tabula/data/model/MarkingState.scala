...
object MarkingState {
  ...
  case object ReleasedForMarking extends MarkingState("ReleasedForMarking") {
    def transitionStates = Set(InProgress, MarkingCompleted)
  }

  // has been downloaded by the marker and is being marked
  case object InProgress extends MarkingState("InProgress") {
    def transitionStates = Set(MarkingCompleted)
  }

  // the uploaded feedback was rejected by a moderator and must be reviewed
  case object Rejected extends MarkingState("Rejected") {
    def transitionStates = Set(ReleasedForMarking, MarkingCompleted)
  }

  // submission has been marked and feedback has been uploaded
  case object MarkingCompleted extends MarkingState("MarkingCompleted") {
    def transitionStates = Set(Rejected)
  }
