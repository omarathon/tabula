...
object Routes {
	...
	object admin {
		...
		object assignment {
			...
			def submissionAttachmentDownload(submission: Submission, filename: String, encodeFilename: Boolean = true): String =
				assignmentroot(submission.assignment) + s"/submissions/download/${submission.id}/" + (if(encodeFilename) s"${encoded(filename)}" else s"$filename") + s"?single=true"
			...
			def extensionAttachment(extension: Extension, filename: String, encodeFilename: Boolean = true): String =
				admin() + s"/extensions/${extension.id}/supporting-file/" + (if(encodeFilename) s"${encoded(filename)}" else s"$filename")
