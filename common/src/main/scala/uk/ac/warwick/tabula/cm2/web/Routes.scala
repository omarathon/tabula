...
object Routes {
	...
	object admin {
		...
		object assignment {
			...
			def submissionAttachmentDownload(submission: Submission, filename: String): String =
				assignmentroot(submission.assignment) + s"/submissions/download/${submission.id}/" + s"${encoded(filename)}?single=true"
			...
			def extensionAttachment(extension: Extension, filename: String, encodeFilename: Boolean = true): String =
				admin() + s"/extensions/${extension.id}/supporting-file/${encoded(filename)}"
