...
object Routes {
	...
	object admin {
		...
		object assignment {
			...
			def submissionAttachmentDownload(submission: Submission, filename: String): String = assignmentroot(submission.assignment) + s"/submissions/download/${submission.id}/${filename}?single=true"
			...
			def extensionAttachment(extension: Extension, filename: String): String = admin() + s"/extensions/${extension.id}/supporting-file/${filename}"
