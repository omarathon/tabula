...
import uk.ac.warwick.tabula.commands.cm2.CourseworkMarkerHomepageCommand._
...
object CourseworkMarkerHomepageCommand {
	...
	type Command = Appliable[CourseworkHomepageMarkerInformation]

	def apply(user: CurrentUser): Command =
		new CourseworkMarkerHomepageCommandInternal(user)
			with ComposableCommand[CourseworkHomepageMarkerInformation]
...
class CourseworkMarkerHomepageCommandInternal(val user: CurrentUser)
	extends CommandInternal[CourseworkHomepageMarkerInformation]
	...
	override def applyInternal(): CourseworkHomepageMarkerInformation = benchmarkTask("Get marker information") {
...
trait CourseworkHomepageMarkerAssignments extends CourseworkHomepageMarkerAssignmentList {
