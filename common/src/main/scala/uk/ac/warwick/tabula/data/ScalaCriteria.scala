...
class ScalaCriteria[A](c: org.hibernate.Criteria) {
  ...
  def add(criterion: Criterion): ScalaCriteria[A] = chainable {
    c.add(criterion)
  }
  ...
  def addOrder(order: Order): ScalaCriteria[A] = chainable {
    c.addOrder(order)
  }
  ...
  def setMaxResults(i: Int): ScalaCriteria[A] = chainable {
    c.setMaxResults(i)
  }

  def setFirstResult(i: Int): ScalaCriteria[A] = chainable {
    c.setFirstResult(i)
  }

  def setFetchMode(associationPath: String, mode: FetchMode): ScalaCriteria[A] = chainable {
    c.setFetchMode(associationPath, mode)
  }
  ...
  def distinct: ScalaCriteria[A] = chainable {
...
class ProjectedScalaCriteria[A, B](c: ScalaCriteria[A]) {
  ...
  def distinct: ProjectedScalaCriteria[A, B] = chainable {
    c.setResultTransformer(DistinctRootEntityResultTransformer.INSTANCE)
  }
