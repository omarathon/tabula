...
class DepartmentSmallGroupIdConverter extends TwoWayConverter[String, DepartmentSmallGroup] {
  ...
  override def convertRight(id: String): DepartmentSmallGroup = (Option(id) flatMap {
    service.getDepartmentSmallGroupById(_)
  }).orNull

  override def convertLeft(group: DepartmentSmallGroup): String = (Option(group) map {
    _.id
  }).orNull
