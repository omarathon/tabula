...
object SmallGroupSetFilters {
  ...
  def allModuleFilters(modules: Seq[model.Module]): Seq[Module] = modules.map {
    Module
  }
  ...
  def allFormatFilters: Seq[Format] = SmallGroupFormat.members.map {
    Format
  }
  ...
  object Status {
    ...
    case object NeedsEventsCreating extends SmallGroupSetFilter {
      ...
      def apply(set: SmallGroupSet): Boolean = set.groups.asScala.forall {
        _.events.isEmpty
      }
    }
  ...
  object AllocationMethod {
    ...
    def all(linked: Seq[DepartmentSmallGroupSet]): Seq[SmallGroupSetFilter with Product with Serializable] = Seq(ManuallyAllocated, StudentSignUp) ++ linked.map {
      Linked
    }
  }
