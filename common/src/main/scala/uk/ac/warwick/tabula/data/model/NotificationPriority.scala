...
object NotificationPriority {
  ...
  case object Complete extends NotificationPriority("complete") {
    def toNumericalValue = 0
  }

  // Notification is trivial; by default this will not generate an email
  case object Trivial extends NotificationPriority("trivial") {
    def toNumericalValue = 0.1
  }

  // Notification is for information only
  case object Info extends NotificationPriority("info") {
    def toNumericalValue = 0.25
  }

  // Notification is about an action that the user should take
  case object Warning extends NotificationPriority("warning") {
    def toNumericalValue = 0.5
  }

  // Notification is about an action that is super important
  case object Critical extends NotificationPriority("critical") {
    def toNumericalValue = 0.75
  }
