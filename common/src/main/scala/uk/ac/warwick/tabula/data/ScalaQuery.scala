...
class ScalaQuery[A](c: org.hibernate.query.Query[A]) {
  ...
  def distinct: ScalaQuery[A] = chainable {
  ...
  def setParameter(name: String, value: Any): ScalaQuery[A] = chainable {
    c.setParameter(name, value)
  }

  def setParameterList(name: String, list: Seq[_]): ScalaQuery[A] = chainable {
    c.setParameterList(name, list.toList.asJava)
  }

  def setMaxResults(maxResults: Int): ScalaQuery[A] = chainable {
    c.setMaxResults(maxResults)
  }

  def setFirstResult(firstResult: Int): ScalaQuery[A] = chainable {
    c.setFirstResult(firstResult)
  }

  // Helper to neaten up the above chainable methods - returns this instead of plain Query
  @inline private def chainable(fn: => Unit): ScalaQuery[A] = {
    fn; this
  }
...
class ScalaUpdateQuery(c: org.hibernate.query.Query[_]) {
  ...
  def setParameter(name: String, value: Any): ScalaUpdateQuery = chainable {
    c.setParameter(name, value)
  }

  def setParameterList(name: String, list: Seq[_]): ScalaUpdateQuery = chainable {
    c.setParameterList(name, list.toList.asJava)
  }

  // Helper to neaten up the above chainable methods - returns this instead of plain Query
  @inline private def chainable(fn: => Unit): ScalaUpdateQuery = {
    fn; this
  }
