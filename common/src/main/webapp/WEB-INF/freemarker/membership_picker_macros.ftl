...
-->
  <#macro fieldset command classifier name enrolment_url>
	...
	<div style="margin-bottom: 8px;">
      ...
      <a class="btn btn-default use-tooltip disabled show-sits-picker"
         title="Use enrolment data from one or more ${name}s recorded in SITS">Add link to SITS</a>
    <#else>
      <a class="btn btn-default use-tooltip disabled"
         title="No ${name}s are recorded for this module in SITS. Add them there if you want to create a parallel link in Tabula.">No SITS link available</a>
    </#if>
      ...
      <div tabindex="0" role="button" class="use-tooltip" style="display: inline-block;"
      ...
      <div tabindex="0" role="button" class="use-tooltip" style="display: inline-block;" title="Re-enrol selected students">
    ...
    <div id="enrolment">
            ...
            <tr class="membership-item item-type-${item.itemTypeString}"> <#-- item-type-(sits|include|exclude) -->
                ...
                <#else>
                  <i tabindex="0" role="button" class="icon-ban-circle fa fa-ban use-tooltip"
                     title="We are missing this person's usercode, without which we cannot modify their enrolment."></i>
                </#if>
							</td>
      ...
      <div class="${classifier}Modals">
        <@modal.modal cssClass="<#if component.name == 'courses'>hide</#if> adder">
          <@modal.wrapper>
            ...
            <@modal.body>
              ...
              <p class="alert">
                <i class="icon-lightbulb fa fa-lightbulb-o icon-large fa fa-lg"></i> <strong>Is your module in SITS?</strong> It may be better to fix the data
                there,
        ...
        <@modal.modal cssClass="<#if component.name == 'courses'>hide</#if> sits-picker">
          ...
          <@modal.body>
            ...
            <table id="sits-table"
                   class="table <#if component.name == 'courses'>table-bordered tabula-orangeLight</#if> table-striped table-condensed table-hover table-sortable table-checkable sticky-table-headers">
              <thead>
              <tr>
                <th class="for-check-all" style="width: 20px; padding-right: 0;"></th>
    ...
    <script type="text/javascript" nonce="${nonce()}">
      jQuery(function ($) {
        ...
        var initEnrolment = function () {
          ...
          setTimeout(function () {
            $('.sb-table-wrapper-popout').remove()
          }, 500);
        ...
        $enrolment.on('click', '.sits-picker .btn', function (e) {
          ...
          if ($(this).is(':not(.disabled)')) {
            ...
            var changes = $m.find('td input:checked:not(:disabled)').map(function (i, input) {
              return input.value
            }).toArray();
