...
<#list relationships?keys as relationshipType>
  - <@fmt.p number=mapGet(relationships, relationshipType)?size singular=relationshipType.agentRole?cap_first shownumber=false/>: <#list mapGet(relationships, relationshipType) as relationship>${relationship.agentName}<#if relationship_has_next>, </#if></#list>
</#list>
