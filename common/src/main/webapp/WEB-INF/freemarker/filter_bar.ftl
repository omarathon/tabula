        ...
        <div class="student-filter btn-group-group well well-small well-sm">
          ...
          <@filter "courseTypes" placeholder currentfilter filterCommand.allCourseTypes; courseType>
            <input type="checkbox" name="${status.expression}" value="${courseType.code}"
                   data-short-value="${courseType.code}" ${contains_by_code(filterCommand.courseTypes, courseType)?string('checked','')}>
          ...
          <@filter path="routes" placeholder=placeholder currentFilter=currentfilter allItems=filterCommand.allRoutes validItems=filterCommand.visibleRoutes customPicker=routesCustomPicker; route, isValid>
            <input type="checkbox" name="${status.expression}" value="${route.code}"
                   data-short-value="${route.code?upper_case}" ${contains_by_code(filterCommand.routes, route)?string('checked','')} <#if !isValid>disabled</#if>>
          ...
          <@filter "sprStatuses" placeholder currentfilter filterCommand.allSprStatuses; sprStatus>
            <input type="checkbox" name="${status.expression}" value="${sprStatus.code}"
                   data-short-value="${sprStatus.shortName?capitalize}" ${contains_by_code(filterCommand.sprStatuses, sprStatus)?string('checked','')}>
            ${sprStatus.fullName}
          </@filter>
