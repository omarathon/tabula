...
class CourseworkAdminTest extends BrowserTest with CourseworkFixtures {
  ...
  "Department admin" should "be offered a link to their department" in as(P.Admin1) {
    ...
    pageSource contains "My department-wide responsibilities" should be {
      true
    }
    ...
    getModule("xxx02").get.isDisplayed should be {
      true
    }
    getModule("xxx03").get.isDisplayed should be {
      true
    }
    ...
    eventually {
      getModule("xxx01").isDefined should be {
        false
      }
      getModule("xxx02").get.isDisplayed should be {
        true
      }
      getModule("xxx03").isDefined should be {
        false
      }
    }
  }
