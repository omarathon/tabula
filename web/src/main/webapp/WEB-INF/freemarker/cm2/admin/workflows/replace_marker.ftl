  ...
  >
    ...
    <@bs3form.labelled_form_group path="oldMarker" labelText="Marker to replace">
      <@f.select path="oldMarker" class="form-control" >
        ...
        <#list replaceMarkerCommand.allMarkers as marker>
          <option <#if replaceMarkerCommand.oldMarker?? && replaceMarkerCommand.oldMarker == marker.userId>selected
                  </#if>value="${marker.userId}">${marker.firstName} ${marker.lastName}</option>
        </#list>
      </@f.select>
    ...
    <#if replaceMarkerCommand.affectedAssignments?has_content>
      ...
      <div class="alert alert-info">
        <p>
          Any students <#if workflow.reusable>to the assignments listed above </#if>that have already been released will not be re-released, so the new marker
          will not be notified that they need to add marks for the students. You should contact the new marker yourself.
        </p>
