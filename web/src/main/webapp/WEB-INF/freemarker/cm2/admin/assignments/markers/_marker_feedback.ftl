<#if command.stage.populateWithPreviousFeedback && !command.currentMarkerFeedback.hasBeenModified>
  <#assign makeChangeForm=true>
<#else>
  <#assign makeChangeForm=false>
</#if>
...
<@f.form method="post" enctype="multipart/form-data" modelAttribute="command" studentid="${command.student.userId}" action=actionUrl cssClass="dirty-check double-submit-protection ajax-form">
  <div class="marking-and-feedback"<#if makeChangeForm> style="display:none;"</#if>>
  ...
  <#if !makeChangeForm>
    <input hidden name="approved" value="true" />
  </#if>
  <div class="buttons form-group">
    <button type="submit" class="btn btn-primary">Save</button>
    <a class="btn btn-default reset marking-and-feedback" href="#" <#if makeChangeForm> style="display:none;"</#if>>Reset</a>
    <a class="btn btn-default cancel marking-and-feedback" href="#" <#if makeChangeForm> style="display:none;"</#if>>Cancel</a>
  </div>
</@f.form>
