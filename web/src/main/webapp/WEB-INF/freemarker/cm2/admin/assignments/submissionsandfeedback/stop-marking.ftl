...
<#escape x as x?html>
  ...
  <@f.form method="post" action="${formAction}" modelAttribute="command">
    ...
    <#else>
      <div class="alert alert-info">
        Marking cannot be stopped for any of the students that you selected. <a href="<@routes.cm2.assignmentsubmissionsandfeedback assignment />">Go back to
          the assignment summary.</a>
      </div>
    </#if>
