      ...
      <#if order.hasFeedback>
        <#if !readOnly>
          <#if order.headerStage.nextStagesDescription?has_content>
            <a class="btn btn-primary must-have-selected must-have-ready-next-stage form-post" href="${markingCompleted}">Confirm selected and send
              to ${order.headerStage.nextStagesDescription?lower_case}</a>
          </#if>
        ...
        <div class="btn-group">
          ...
          <ul class="dropdown-menu">
            ...
            <li class="must-have-selected">
              <a class="download-pdf" data-target="#download-pdf-modal-${order.headerStage.name}"
                 href="<@routes.cm2.downloadMarkerSubmissionsPdf assignment marker />">
                Download all selected submissions as pdf
              </a>
            </li>
            <#if features.feedbackTemplates && assignment.hasFeedbackTemplate>
              <li>
                <a class="use-tooltip" title="Download feedback templates for all students as a ZIP file."
                   href="<@routes.cm2.markerTemplatesZip assignment marker/>" data-container="body">
            ...
            <#list workflowType.allPreviousStages(order.headerStage) as pStage>
              <li class="must-have-selected">
                <a class="form-post" href="<@routes.cm2.downloadMarkerFeedbackStage assignment marker pStage />">Download feedback
                  from ${pStage.description?lower_case}</a>
              </li>
            </#list>
          </ul>
        </div>
