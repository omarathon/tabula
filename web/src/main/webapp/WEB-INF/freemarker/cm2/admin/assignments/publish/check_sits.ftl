...
<#escape x as x?html>
  ...
  <@cm2.assignmentHeader "Check Upload to SITS status" assignment />
    ...
    <div class="alert alert-info">
      ...
      <div class="error-message hidden">
        ...
        <ul>
          ...
          <li>MOD_CODE that starts with: ${assignment.module.code?upper_case}</li>
  ...
  <div class="final-status hidden">
    <#if feedbackForSits??>
      <p>We couldn't find a reason why this mark could not be uploaded.
        Try uploading to SITS again and if the problem persists contact us using the "Need help?" button above.</p>
    <#else>
      ...
      var showNextStatus = function () {
        ...
        setTimeout(function () {
          if ($status.hasClass('error')) {
            $status.find('.alert').removeClass('alert-info').addClass('alert-danger').prepend(' ').prepend($('<i/>').addClass('fa fa-times'));
            $status.find('.error-message').removeClass('hidden');
          } else {
            $status.find('.alert').removeClass('alert-info').addClass('alert-success').prepend(' ').prepend($('<i/>').addClass('fa fa-check'));
