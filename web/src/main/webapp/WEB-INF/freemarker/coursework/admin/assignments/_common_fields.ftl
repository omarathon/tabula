...
<#if features.markingWorkflows && command.allMarkingWorkflows?has_content>
  ...
  <@form.labelled_row "markingWorkflow" "Marking workflow">
    <#if !disabled>
      ...
      <div class="help-block">
        Marking workflows define how and by whom the assignment will be marked. They are set up for the department by a Departmental
        Administrator <#if can.do("MarkingWorkflow.Manage", department)><a href="<@routes.coursework.markingworkflowlist department />">here</a></#if>.
      </div>
    <#else>
