  ...
  <#if adjustment??>
    <div class="alert">
      ...
      <#if adjustment_has_next>
        <#if adjustments[adjustment_index + 1].mark??>
          <div>Mark: ${adjustments[adjustment_index + 1].mark}%</div></#if>
        <#if adjustments[adjustment_index + 1].grade??>
          <div>Grade: ${adjustments[adjustment_index + 1].grade}</div></#if>
      <#else>
...
<#if feedback.comments??>
  ...
  <#if feedback.attachments?has_content>
    <div class="feedback-notes">
      <h4>Please note there <@fmt.p number=feedback.attachments?size singular="is" plural="are" shownumber=false />
        also <@fmt.p number=feedback.attachments?size singular="feedback file" plural="feedback files" shownumber=true /> available for download.</h4>
    </div>
  </#if>
