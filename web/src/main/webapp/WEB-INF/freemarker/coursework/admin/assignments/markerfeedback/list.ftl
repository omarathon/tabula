      ...
      <tr class="item-container ${progressClass}"
        ...
        <#if assignment.module.adminDepartment.showStudentName>
          ...
          <td class="student-col toggle-cell">
            <h6>${item.student.lastName} <#if item.student.warwickId??><@pl.profile_link item.student.warwickId /><#else><@pl.profile_link item.student.userId /></#if></h6>
          </td>
          <#assign toggleIcon = "" />
        <#else>
            ...
            <dt>
              ...
              <#else>
                <#if item.submission.late>
                  <span tabindex="0" role="button" class="label label-important use-tooltip" title="<@sd.lateness item.submission />" data-container="body">Late</span>
                </#if>
