...
<#if !assignment.openEnded>
  <#macro extensionButtonContents label assignment>
    <a href="<@routes.coursework.extensionRequest assignment=assignment />?returnTo=<@routes.coursework.assignment assignment=assignment />"
       class="btn btn-mini">
  ...
  <#elseif assignment.closed>
    <p class="late deadline">
      ...
      <#if hasActiveExtension>
        ...
        <span class="time-remaining">${extension_time_remaining} <span tabindex="0" role="button" class="label label-warning use-tooltip" title=${latenesstooltip} data-container="body">Late</span></span>
        Extension deadline was <@fmt.date date=extension.expiryDate />
      <#else>
        <#assign latenesstooltip><#if isSelf>"<@sd.lateness submission assignment user />"<#else>"<@sd.lateness submission assignment student />"</#if></#assign>
        <span class="time-remaining">${time_remaining} <span tabindex="0" role="button" class="label label-warning use-tooltip" title=${latenesstooltip} data-container="body">Late</span></span>
