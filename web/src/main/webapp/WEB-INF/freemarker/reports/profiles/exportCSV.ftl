...
<#if jobId??>
	...
	<h2>CSV file generating progress</h2>

	<div class="progress">
		<div class="progress-bar progress-bar-striped active" style="width: 0;"></div>
	</div>

	<p class="report-progress">Progressing</p>
	...
	<div class="report-complete alert alert-info" style="display: none;">
		...
		<p><a href="<@routes.reports.profileExportCSVZip department academicYear jobId />" class="btn btn-default"><i class="fa fa-arrow-circle-o-down"></i> Download reports as a .zip file</a></p>
	</div>

	<script>
		jQuery(function($){
			var updateProgress = function() {
				$.get('<@routes.reports.profileExportCSVProgress department academicYear jobId />', function(data){
					if (data.succeeded) {
