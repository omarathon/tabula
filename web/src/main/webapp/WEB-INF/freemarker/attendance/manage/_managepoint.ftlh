    ...
    <div class="col-md-5">
      <@bs3form.labelled_form_group path="meetingRelationships" labelText="Meeting with">
        <#list command.department.displayedStudentRelationshipTypes as relationship>
          <@bs3form.checkbox>
            <input type="checkbox" name="meetingRelationships" id="meetingRelationships-${relationship.urlPart}" value="${relationship.urlPart}"
                   <#if meetingRelationshipsStrings?seq_contains(relationship.urlPart)>checked</#if> />
    ...
    <div class="col-md-6">
      ...
      <@bs3form.labelled_form_group path="meetingFormats" labelText="${label}">
        <#list allMeetingFormats as format>
          <@bs3form.checkbox>
            <input type="checkbox" name="meetingFormats" id="meetingFormats-${format.code}" value="${format.code}"
                   <#if meetingFormatsStrings?seq_contains(format.code)>checked</#if> />
            ${format.description}
          </@bs3form.checkbox>
        </#list>
      </@bs3form.labelled_form_group>
  ...
  <div class="pointTypeOption smallGroup" <#if ((command.pointType.dbValue)!'null') != 'smallGroup'>style="display:none"</#if>>
      ...
      <@bs3form.labelled_form_group path="smallGroupEventModules" labelText="Modules">
        ...
        <@bs3form.radio>
          <input class="specific" type="radio" <#if (!command.anySmallGroupEventModules)>checked </#if> value="false" name="isAnySmallGroupEventModules"
                 data-selector=".specific-module-search" />
          Specific
              ...
              <ol>
                <#list command.smallGroupEventModules![] as module>
                  <li>
                    ...
                    <#if command.moduleHasSmallGroups(module)><i class="fa fa-fw"></i><#else><i class="fa fa-fw fa-exclamation-circle"
                                                                                                title="This module has no small groups set up in Tabula"></i></#if>
                    <span title="<@fmt.module_name module false />"><@fmt.module_name module false /></span>
                    <button class="btn btn-danger btn-xs remove">Remove</button>
                  </li>
                </#list>
              </ol>
            </div>
    ...
    <@bs3form.labelled_form_group path="smallGroupEventQuantityAll" labelText="${label}">
      <input class="form-control" type="text"
             <#if (command.smallGroupEventQuantity?? && command.smallGroupEventQuantity > 0)>value="${command.smallGroupEventQuantity}"</#if>
             name="smallGroupEventQuantity" />
    </@bs3form.labelled_form_group>
  ...
  <div class="pointTypeOption assignmentSubmission" <#if ((command.pointType.dbValue)!'null') != 'assignmentSubmission'>style="display:none"</#if>>
    ...
    <div class="assignmentSubmissionType-any" <#if command.assignmentSubmissionType != 'any'>style="display:none"</#if>>
      ...
      <@bs3form.labelled_form_group path="assignmentSubmissionTypeAnyQuantity" labelText="${label}">
        <input class="form-control" type="text"
               <#if (command.assignmentSubmissionTypeAnyQuantity?? && command.assignmentSubmissionTypeAnyQuantity > 0)>value="${command.assignmentSubmissionTypeAnyQuantity}"</#if>
               name="assignmentSubmissionTypeAnyQuantity" />
      </@bs3form.labelled_form_group>
    </div>
    ...
    <div class="assignmentSubmissionType-modules" <#if command.assignmentSubmissionType != 'modules'>style="display:none"</#if>>
            ...
            <ol>
              <#list command.assignmentSubmissionModules![] as module>
                <li>
                  ...
                  <#if command.moduleHasAssignments(module)><i class="fa fa-fw"></i><#else><i class="fa fa-fw fa-exclamation-circle"
                                                                                              title="This module has no assignments set up in Tabula"></i></#if>
                  <span title="<@fmt.module_name module false />"><@fmt.module_name module false /></span>
                  <button aria-label="Remove" class="btn btn-danger btn-xs remove"><i class="fa fa-times"></i></button>
                </li>
              </#list>
            </ol>
          </div>
      ...
      <@bs3form.labelled_form_group path="assignmentSubmissionTypeModulesQuantity" labelText="${label}">
        <input class="form-control" type="text"
               <#if (command.assignmentSubmissionTypeModulesQuantity?? && command.assignmentSubmissionTypeModulesQuantity > 0)>value="${command.assignmentSubmissionTypeModulesQuantity}"</#if>
               name="assignmentSubmissionTypeModulesQuantity" />
      </@bs3form.labelled_form_group>
    </div>
            ...
            <ol>
              <#list command.assignmentSubmissionAssignments![] as assignment>
                <li>
                  ...
                  <span title="<@fmt.assignment_name assignment false /> (${assignment.academicYear.toString})"><@fmt.assignment_name assignment false /> (${assignment.academicYear.toString})</span>
                  <button aria-label="Remove" class="btn btn-danger btn-xs remove"><i class="fa fa-times"></i></button>
                </li>
              </#list>
            </ol>
