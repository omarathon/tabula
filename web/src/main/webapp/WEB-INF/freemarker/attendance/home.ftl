    ...
    <ul>
      <#list relationshipTypesMap?keys as relationshipType>
        <#if relationshipTypesMapById[relationshipType.id]>
          <li><h3><a id="relationship-${relationshipType.urlPart}"
                     href="<@routes.attendance.agentHomeForYear relationshipType academicYear />">${relationshipType.studentRole?cap_first}s</a></h3></li>
        </#if>
      </#list>
    </ul>
  </#if>
