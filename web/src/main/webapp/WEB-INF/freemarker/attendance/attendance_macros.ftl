        ...
        <tr class="${item.itemTypeString}">
          ...
          <#if checkboxName?has_content>
            <td>
              ...
              <#else>
                <input type="checkbox" class="collection-checkbox" disabled />
                <i tabindex="0" class="fa fa-info-circle use-tooltip" title="Student can only be removed from 'Manually add students' tab below"></i>
              </#if>
            </td>
          </#if>
        </tr>
...
<#macro checkpointLabel department checkpoint="" point="" student="" note="" urlProfile=false>
  ...
  <span tabindex="0" role="button" class="use-popover label ${formatResult.labelClass}" data-content="${popoverContent}" data-html="true" data-trigger="click" data-container=""
...
<#macro checkpointSelect department id name checkpoint="" point="" student="" note="">
  ...
  >
    ...
    <option <#if startDateInFuture>class="disabled" title="This event hasn't happened yet so it can't be marked as missed (unauthorised)" </#if>
            value="unauthorised" <#if checkpoint?has_content && checkpoint.state.dbValue == "unauthorised">selected</#if>>Missed (unauthorised)
    </option>
    ...
    <option <#if startDateInFuture>class="disabled" title="This event hasn't happened yet so it can't be marked as attended" </#if>
            value="attended" <#if checkpoint?has_content && checkpoint.state.dbValue == "attended">selected</#if>>Attended
    </option>
  </select>
</#macro>
...
<#macro checkpointIcon department checkpoint="" point="" student="" note="">
  ...
  <#local popoverContent>
    <#if nonActivePoint>
      <p>
        ...
        <a href='http://warwick.ac.uk/tabula/manual/monitoring-points/recording-viewing-points/#nonactivepoints' target='_blank' aria-label='Help'><i
  ...
  <span tabindex="0" role="button" class="fa fa-fw fa-stack fa-stack-original-size fa-stack-right use-popover" data-content="${popoverContent}" data-html="true" data-trigger="click" data-container="">
        ...
        <div class="left">
          <table class="students table table-striped table-condensed">
            ...
            <tr>
              ...
              <th class="unrecorded-col <#if doCommandSorting>${sortClass("attendanceCheckpointTotals.unrecorded", command)}</#if> sortable"
                  data-field="attendanceCheckpointTotals.unrecorded">
                <i title="Unrecorded" class="fa fa-fw fa-exclamation-triangle late"></i>
              </th>
              <th class="missed-col <#if doCommandSorting>${sortClass("attendanceCheckpointTotals.unauthorised", command)}</#if> sortable"
                  data-field="attendanceCheckpointTotals.unauthorised">
