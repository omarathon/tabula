...
<#macro event_schedule_info event>
  <#if event.unscheduled>
    <span tabindex="0" class="badge progress-bar-warning use-tooltip" data-toggle="tooltip" data-placement="bottom" data-title="This event has not yet been scheduled">Not scheduled</span>
  <#else>
  ...
  <span id="${set_anchor(set)}-container">
        ...
        data-href="<@routes.groups.single_set_info set />"
            ...
            <#if set.archived>
              ...
              <dl tabindex="0" class="progress use-tooltip" title="${progressTooltip}" style="margin: 0; border-bottom: 0;" data-container="body">
                  ...
                  <li tabindex="0" <#if !set.canBeDeleted> class="disabled use-tooltip" data-container="body" title="Can't delete smallgroups that have been published to <#if set.allocationMethod.dbValue != 'StudentSignUp'>students or </#if>tutors"</#if>>
              ...
              <a href="<@routes.groups.deregisteredStudents set />?returnTo=${info.requestedUri}<#if filterQuery??>?${filterQuery?url}</#if>"
                 class="use-tooltip warning" style="display: inline;" title="${tooltip}" aria-label="${tooltip}"><i
                        class="fa fa-exclamation-triangle warning"></i></a>
            </#if>
            </#if>
					</div>
    ...
    <#if unallocatedSize gt 0 || (notInMembershipCount gt 0 && can_update_groups)>
      <div class="col-md-8">
        <#if unallocatedSize gt 0>
          ...
          <#else>
            <a href="<@routes.groups.unallocatedstudentslist set />" class="ajax-modal"
               data-target="#students-list-modal"><@fmt.p unallocatedSize "unallocated student" /></a>
          </#if>
        </#if>
  ...
  <span id="${module_anchor(module)}-container">
    ...
    <#list moduleItem.setItems as setItem>
      <#if setItem.set.archived>
		...
		<div id="${module_anchor(module)}"
         class="module-info striped-section<#if has_groups> collapsible<#if expand_by_default> expanded</#if></#if><#if canManageDepartment && !has_groups> empty</#if>"
      ...
      <form id="select-signup-${setItem.set.id}" method="post" action="<@routes.groups.signup_to_group setItem.set />">
        ...
        <div class="${moduleItem.canManageGroups?string('col-md-8','col-md-10')}">
          ...
          <#list setItem.groups as groupItem>
            ...
            <div class="row group">
              <div class="col-md-12">
                <#if setItem.viewerMustSignUp>
                <div <#if group.full>
                    tabindex="0" class="pull-left use-tooltip" title="There are no spaces left on this group"
                  <#else>
                    class="pull-left"
                  </#if>
                >
                      ...
                      <li class="clearfix">
                        ...
                        <div class="eventWeeks">
                          <#if (showOccurrenceAttendance!false) && eventItem.occurrences?has_content && groupSet.collectAttendance>
                            <a href="<@routes.groups.registerForWeek eventItem.event, eventItem.occurrences?first.week />"
          ...
          <#if setItem.viewerMustSignUp>
            ...
            <a data-href="<@routes.groups.signup_to_group_timetableclash_info setItem.set />" class="timetable-clash-link" data-toggle="modal"
               data-target="#timetable-clash-modal"></a>
          </#if>
          ...
          <#if moduleItem.canManageGroups>
            ...
            <#if unallocatedSize gt 0>
              <div class="alert">
                <a href="<@routes.groups.unallocatedstudentslist setItem.set />" class="ajax-modal"
                   data-target="#students-list-modal"><@fmt.p unallocatedSize "student has" "students have" /> not been allocated to a group</a>
              </div>
            </#if>
...
<#macro studentAttendanceRow student attendance notes instances group>
  ...
  <tr>
    <#list instances as instance>
      ...
      <td>
				<span tabindex="0" role="button" class="fa-stack fa-stack-original-size fa-stack-right fa-fw use-popover" data-trigger="click focus" data-content="${renderedTitle?markup_string}" data-html="true">
...
<#macro eventDetails event><#compress>
  <#if event.title?has_content>
    <div class="eventTitle">${event.title}</div></#if>
  <div class="day-time">
    ${(event.day.name)!""}
