  ...
  <@modal.wrapper enabled=isModal>
    ...
    <@modal.body enabled=isModal>
      <p>
        ...
        ${attendanceNote.occurrence.event.day.name} <@fmt.time attendanceNote.occurrence.event.startTime />
        - <@fmt.time attendanceNote.occurrence.event.endTime />,
      ...
      <#if attendanceNote.attachment?has_content>
        <p>
          <@fmt.download_link
          filePath="/groups/note/${attendanceNote.student.universityId}/${attendanceNote.occurrence.id}/attachment/${attendanceNote.attachment.name}"
