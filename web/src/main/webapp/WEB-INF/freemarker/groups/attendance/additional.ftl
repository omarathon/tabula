<#escape x as x?html>
  ...
  <form action="" method="post">
    <@modal.wrapper>
      <@modal.header>
        <h6 class="modal-title">Add ${student.fullName} to <#if event.title?has_content>${event.title}, </#if>${event.group.groupSet.name}
          , ${event.group.name}</h6>
      </@modal.header>
      <@modal.body>
        ...
        <select id="replacementEventAndWeek" class="form-control">
          ...
          <#list possibleReplacements as replacement>
            <option value="${replacement.event.id}_${replacement.week}" data-event="${replacement.event.id}" data-week="${replacement.week}">
              ...
              ${replacement.event.day.name} <@fmt.time replacement.event.startTime /> - <@fmt.time replacement.event.endTime />
              , <@fmt.singleWeekFormat week=replacement.week academicYear=replacement.event.group.groupSet.academicYear dept=replacement.event.group.groupSet.module.adminDepartment />
