...
<head>
  <style type="text/css">
    ...
    <#if showPhotos> padding-top: 10px;
      padding-bottom: 5px;
      min-height: 40px;
    <#else> padding-top: 7px;
      padding-bottom: 5px;
    ...
    .student .check-box, .student .signature-line {
      display: inline-block;
      vertical-align: middle;
      text-align: right;
      width: 35%;
    ...
    .student .check-box input[type="checkbox"] {
      width: 30px;
      height: 30px;
    }
...
<div id="header">
  ...
  <h2 class="sgt-event-details">
    ${event.day.name} <@fmt.time event.startTime /> - <@fmt.time event.endTime />
    , <@fmt.singleWeekFormat week=week academicYear=set.academicYear dept=set.module.adminDepartment /> (${formattedEventDate})
