...
<#escape x as x?html>
  ...
  <div class="fix-area">
    <@f.form method="post" id="import-form" action="${post_url}" modelAttribute="command">
      ...
      <p>Use the checkboxes at the left-hand side to choose the groups to import to Tabula. If a group set with the same name already exists in the selected
        academic year, it is not selected by default. However, you can select these to import them again (they are imported as a separate group set).</p>

      <p>Lectures are not selected by default. If you want to record attendance at lectures using Tabula, use the checkboxes at the left-hand side to choose
        which lectures to import. Note that lectures always appear on students' timetables, whether you import them or not.</p>
