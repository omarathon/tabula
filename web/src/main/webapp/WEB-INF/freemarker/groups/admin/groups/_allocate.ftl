    ...
    <div class="tab-content">
          ...
          <@f.form method="post" action="${submitUrl}" modelAttribute="command" class="allocateStudentsToGroupsCommand dirty-check">
                      ...
                      <div class="drag-target well clearfix group-${group.id}">
                        <div class="group-header">
                          ...
                          <div>
                            ...
                            <span class="drag-count">${count}</span> <span class="drag-counted" data-singular="student"
                                                                           data-plural="students">student<#if count != 1>s</#if></span>

                            <a id="show-list-${group.id}" class="show-list" title="View students" data-container=".group-${group.id}"
                               data-title="${popoverHeader}" data-prelude="${groupDetails}" data-placement="left" aria-label="${popoverHeader}"><i
                                      class="fa fa-pencil-square-o"></i></a>
                          </div>
                        </div>

                        <ul class="drag-list hide" data-bindpath="mapping[${group.id}]"
                            <#if group.maxGroupSize??>data-max-members="${group.maxGroupSize}"</#if>>
                    ...
                    <div class="alert alert-info clash-info-alert">
                      ...
                      <div class="clash-info hide">
                        <div class="clash-singular-info"><span class="clash-count"></span> student has small group event conflicts with this allocation.
                          <a href="#" data-href="<@routes.groups.timetableclashstudentslist command.set />" class="ajax-modal timetable-clash-link"
                             data-target="#timetable-clash-modal">See student</a>
                        </div>
                        <div class="clash-plural-info"><span class="clash-count"></span> students have small group event conflicts with this allocation.
                          <a href="#" data-href="<@routes.groups.timetableclashstudentslist command.set />" class="ajax-modal timetable-clash-link"
                             data-target="#timetable-clash-modal">See students</a>
                        </div>
                      </div>
            ...
            <div class="fix-footer">
              ...
              <a class="btn btn-default dirty-check-ignore"
                 href="<@routes.groups.depthome module=smallGroupSet.module academicYear=smallGroupSet.academicYear/>">Cancel</a>
            </div>
          </@f.form>
        </div>
      </div><!-- end 1st tab -->
      ...
      <div class="tab-pane" id="allocategroups-tab2">
        ...
        <@f.form method="post" enctype="multipart/form-data" action="${submitUrl}" modelAttribute="command" cssClass="dirty-check">
          ...
          <ol>
            <li><strong><a href="<@routes.groups.templatespreadsheet smallGroupSet />">Download a template spreadsheet</a></strong>. This will be prefilled with
              the names and University ID numbers of students you have selected to be in ${smallGroupSet.name}. In Excel you may need to <a
                      href="https://support.office.com/en-us/article/what-is-protected-view-d6f09ac7-e6b9-4495-8e43-2bbcdbcb6653#bm2">exit protected view</a> to
              edit the spreadsheet.
            </li>
            <li><strong>Allocate students</strong> to groups using the dropdown menu in the <strong>Group name</strong> column or by pasting in a list of group
              names. The group names must match the groups you have already created for ${smallGroupSet.name}. The <strong>group_id</strong> field will be
              updated with a unique ID number for that group.
              You can select additional students to be in ${smallGroupSet.name} by entering their University ID numbers in the <strong>student_id</strong>
              column. Any students with an empty group_id field will be added to the list of students who haven't been allocated to a group.
            </li>
      ...
      var clashInfo = function () {
        ...
        if ($('.loading').hasClass('hide')) {
          ...
          if (totalClashes > 0) {
            ...
            $timtableClashLink.attr('href', $timtableClashLink.data('href') + encodeURI(clashUserIds));
          } else {
