  ...
  <div class="fix-area">
    ...
    <@f.form method="post" action="${submitUrl}.pdf" modelAttribute="command">
      ...
      <@bs3form.labelled_form_group path="startDate" labelText="Start date">
        <@f.input path="startDate" cssClass="form-control date-picker" autocomplete="off" />
      </@bs3form.labelled_form_group>

      <@bs3form.labelled_form_group path="endDate" labelText="End date">
        <@f.input path="endDate" cssClass="form-control date-picker" autocomplete="off" />
      </@bs3form.labelled_form_group>
      ...
      <table class="table table-striped small-group-sets-list">
        ...
        <#list command.smallGroupsInDepartment as set>
          <tr>
            <td><input class="collection-checkbox" type="checkbox" name="smallGroupSets" value="${set.id}"
                       <#if command.smallGroupSetIds?seq_contains(set.id)>checked</#if> /></td>
