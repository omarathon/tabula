...
<#if features.markingWorkflows>
  <@bs3form.labelled_form_group path="markingWorkflow" labelText="Marking workflow">
    ...
    <div class="help-block">
      ...
      <#else>
        Marking workflows define how and by whom the exam will be marked. They are set up for the department by a Departmental Administrator <#if can.do("MarkingWorkflow.Manage", department)>
        <a href="<@routes.exams.markingWorkflowList department />">here</a></#if>.
      </#if>
    </div>
  </@bs3form.labelled_form_group>
</#if>
