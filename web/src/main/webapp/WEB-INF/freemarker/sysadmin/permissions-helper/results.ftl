...
<#if results.scopeMismatch>
  <div class="alert <#if permissionsHelperCommand.permission.scoped>alert-block<#else>alert-danger</#if>">
    ...
    <#if permissionsHelperCommand.permission.scoped>
      <p><span class="permission"><i class="fa fa-lock"></i> ${permissionsHelperCommand.permission.name}</span> is a scoped permission and can only be resolved
        against a provided scope.</p>
    </#if>
  </div>
</#if>
...
<div class="well">
  <table class="table">
    ...
    <tr>
      <th>Explicit granted permissions<br>
        <small>(relevant to scope)</small>
      </th>
      <td>
