<#compress><#escape x as x?html>
  ...
  <table class="table table-striped table-condensed roles-table">
    ...
    <th></th>
    <#list (rolesTable?first)._2() as roles>
      <th class="rotated" data-name="${roleId(roles._1())}">
        <div class="rotate"><#noescape>${roleDescription(roles._1())}</#noescape></div>
      </th>
    </#list>
    </thead>
    <tbody>
    <#list rolesTable as permission>
      <tr>
        ...
        <td><abbr tabindex="0" role="button" class="use-tooltip" title="${permission._1().description}">${permissionName}</abbr></td>
          ...
          <td data-name="${roleId(roles._1())}">
            <#if roles._2()?has_content>
              ...
              <#else>
                ...
                <#assign title><#noescape>${roleDescription(roles._1())}</#noescape>
                  <strong>cannot</strong> ${permission._1().description?uncap_first}</#assign>
              </#if>
            <#else>
              <#assign icon="icon-ok-circle fa fa-check-circle-o authorised has-role" />
              <#assign title><#noescape>${roleDescription(roles._1())}</#noescape> <strong>can</strong> ${permission._1().description?uncap_first} <strong>for
                students with the same relationship</strong></#assign>
            </#if>

            <i tabindex="0" role="button" class="${icon} icon-fixed-width fa fa-fw use-tooltip" title="${title}" data-html="true"></i>
          </td>
        </#list>
      </tr>
    </#list>
    ...
    jQuery(function ($) {
      ...
      $('.show-hide-form input[type="checkbox"]').on('change', function () {
        ...
        var hasData = function () {
          return $(this).find('.has-role:visible').length > 0;
        };

        $('.roles-table tbody tr').show().not(hasData).hide();
      });

      $('.show-hide-form label').on('click', function (e) {
        e.stopPropagation();
      });
    });
  </script>
