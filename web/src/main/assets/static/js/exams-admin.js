  ...
  $('input[name=showAdditionalYears]').on('change', function () {
    ...
    if ($this.is(':checked')) {
      ...
      $editor
        ...
        .each(function () {
          $(this).find('.col-xs-2').filter(function (i) {
            return i > 5;
          }).hide();
        });
    ...
    if (academicYear.length > 0) {
      ...
      $.get($academicYearSelect.data('href') + academicYear, function (data) {
        ...
        if (codes.length === 0) {
          ...
          var yearsOfStudy = _.sortBy(_.keysIn(data[codes[0]]), function (year) {
...
function editorFilterAndBulkEdit($editor, inputName) {
  ...
  $editor.on('keyup', 'input[name=filter]', function () {
    ...
    if (filterVal.length === 0) {
      ...
      stripeElements($rows.hide().filter(function () {
        ...
        return _.every(terms, function (val) {
    ...
    if ($editor.find('input[name=bulk]').filter(function () {
  ...
  $applyButton.on('click', function () {
    $editor.find('input[name=bulk]').filter(function () {
      return $(this).val().length > 0
    }).each(function () {
