...
class GenerateModuleExamGridController extends ExamsController
  ...
  def selectExamModuleSubmit(
    ...
    if (students.isEmpty) {
      ...
      if (!maintenanceModeService.enabled) {
        ...
        val jobInstance = jobService.add(Some(user), ImportMembersJob(
          students.map(_.moduleRegistration.studentCourseDetails.student.universityId),
          Seq(academicYear)
        ))
  ...
  def checkJobProgress(
    ...
    if (jobInstance.isDefined && !jobInstance.get.finished) {
      ...
      val (oldestImport, studentLastImportDates) = benchmarkTask("gatherLastImportDateModuleGridData") {
