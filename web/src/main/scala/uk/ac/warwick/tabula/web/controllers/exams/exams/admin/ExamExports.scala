...
class XMLBuilder(val students: Seq[User], val results: ViewExamCommandResult, val exam: Exam, val module: Module, val academicYear: AcademicYear)
  ...
  def toXML: Elem = {
    <exam>
      ...
      <department-name>
        {module.adminDepartment.name}
      </department-name>
      <department-code>
        {module.adminDepartment.code}
      </department-code>
      <academic-year>
        {academicYear}
      </academic-year>
  ...
  def studentXML(student: User): Elem = {
    ...
    <student>
      ...
      <seat-number>
        {results.seatNumberMap.get(student).flatten.getOrElse("")}
      </seat-number>
      <original-mark>
        {if (hasFeedback) {
        feedback.actualMark.getOrElse("")
      }}
      </original-mark>
      <original-grade>
        {if (hasFeedback) {
        feedback.actualGrade.getOrElse("")
      }}
      </original-grade>
      <adjusted-mark>
        {if (hasFeedback) {
        feedback.latestPrivateOrNonPrivateAdjustment.map(_.mark).getOrElse("")
      }}
      </adjusted-mark>
      <adjusted-grade>
        {if (hasFeedback) {
        feedback.latestPrivateOrNonPrivateAdjustment.flatMap(_.grade).getOrElse("")
      }}
      </adjusted-grade>
      <sits-upload-status>
        {if (hasSitsStatus) {
      ...
      <sits-upload-date>
        {if (hasSitsStatus) {
        sitsStatus.dateOfUpload
      }}
      </sits-upload-date>
      <sits-upload-mark>
        {if (hasSitsStatus) {
        sitsStatus.actualMarkLastUploaded
      }}
      </sits-upload-mark>
      <sits-upload-grade>
        {if (hasSitsStatus) {
        sitsStatus.actualGradeLastUploaded
      }}
      </sits-upload-grade>
    </student>
  }
